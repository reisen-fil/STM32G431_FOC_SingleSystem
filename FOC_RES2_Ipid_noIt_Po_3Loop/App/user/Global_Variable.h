...
{
	float kp,ki,kd;
	float target,error,error_last,value,erro_sum,erro_bias,errSumMax,out;
	
}Str_pid;

extern Str_pid Id_pid;   /* 电流环pid */
extern Str_pid Iq_pid;
...
typedef struct 
{
    ...
    float previous_angle_rad;     // 上一次角度（弧度）
    float angle_diff_rad;         // 角度变化量（弧度）

	uint32_t Delta_cnt_diff;	  // 时间变化

	float speed_rad_per_s;        // 角速度（rad/s）
