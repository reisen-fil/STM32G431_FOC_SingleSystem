...
/* 位置环pid */
void Po_PD_Cal(Str_pid *PD_Control, float target, float outMax)
{
    PD_Control->target = target;
    PD_Control->error = PD_Control->target - PD_Control->value;							//误差等于目标值减去测量值 
//    PI_Control->erro_sum += PI_Control->error;							//积分项累加
//		PI_Control->erro_sum = LimitAmp(PI_Control->erro_sum,PI_Control->errSumMax,-PI_Control->errSumMax);		//积分项累加	
	PD_Control->erro_bias = PD_Control->error - PD_Control->error_last;
    PD_Control->out = LimitAmp(PD_Control->kp * PD_Control->error + PD_Control->kd * (PD_Control->erro_bias),
								outMax,-outMax);		//得到PD运算结果
	
    PD_Control->error_last = PD_Control->error;
